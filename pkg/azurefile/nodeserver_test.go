	...
	tests := []struct {
		...
		},
		{
			...
			req: csi.NodeStageVolumeRequest{VolumeId: "vol_1##", StagingTargetPath: sourceTest,
				...
				Secrets:          secrets},
			execScripts: []ExecArgs{
				{"blkid", []string{"-p", "-s", "TYPE", "-s", "PTTYPE", "-o", "export", fmt.Sprintf("%s/test_disk", proxyMountPath)}, "", &testingexec.FakeExitError{Status: 2}},
				{"mkfs.ext4", []string{"-F", "-m0", fmt.Sprintf("%s/test_disk", proxyMountPath)}, "", fmt.Errorf("formatting failed")},
			},
			expectedErr: testutil.TestError{
				DefaultError: status.Errorf(codes.Internal, "could not format %#v and mount it at \"%s\"", sourceTest, fmt.Sprintf("%s/test_disk", proxyMountPath)),
				// todo: Not a desired error. This will need a better fix
		...
		},
		{
			...
			req: csi.NodeStageVolumeRequest{VolumeId: "vol_1##", StagingTargetPath: sourceTest,
				...
				Secrets:          secrets},
			execScripts: []ExecArgs{
				{"blkid", []string{"-p", "-s", "TYPE", "-s", "PTTYPE", "-o", "export", fmt.Sprintf("%s/test_disk", proxyMountPath)}, "", nil},
				{"mkfs.ext4", []string{"-F", "-m0", fmt.Sprintf("%s/test_disk", proxyMountPath)}, "", nil},
			},
			expectedErr: testutil.TestError{
				// todo: Not a desired error. This will need a better fix
				WindowsError: fmt.Errorf("prepare stage path failed for %s with error: could not cast to csi proxy class", proxyMountPath),
