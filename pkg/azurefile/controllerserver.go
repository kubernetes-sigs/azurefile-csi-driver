...
func (d *Driver) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if len(req.GetSecrets()) == 0 && accountName == "" {
		if v, ok := d.volMap.Load(volName); ok {
			...
			lockKey = fmt.Sprintf("%s%s%s%s%s%s%s%v%v%v%v%v%v", sku, accountKind, resourceGroup, location, protocol, subsID, accountAccessTier,
				ptr.Deref(createPrivateEndpoint, false), ptr.Deref(allowBlobPublicAccess, false), ptr.Deref(requireInfraEncryption, false),
				ptr.Deref(enableLFS, false), ptr.Deref(disableDeleteRetentionPolicy, false), ptr.Deref(requiresSmbOAuth, false))
