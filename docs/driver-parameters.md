...
networkEndpointType | specify network endpoint type for the storage account created by driver. This parameter is only applied for NFS protocol. If `privateEndpoint` is specified, a private endpoint will be created for the storage account. For other cases, a service endpoint will be created by default. | "",`privateEndpoint` | No | ``
