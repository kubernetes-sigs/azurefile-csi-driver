...
networkEndpointType | specify the network endpoint type to be created for the storage account. This parameter should only be used in the dynamic provisioning scenarios when a new storage account has to be created and the protocol is nfs. If the value of this parameter is 'privateEndpoint', a private endpoint will be created for the storage account. For any other values or empty value of this parameter, a service endpoint will be created by default if the protocol is nfs. | "","privateEndpoint" | No | ""
