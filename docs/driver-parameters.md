...
allowSharedKeyAccess | Allow or disallow shared key access for storage account created by driver. Applies to both SMB and NFS accounts. <br><br>**NFS:** safe to disable — NFS mount does not use the account key. <br><br>**SMB:** disabling shared key access is supported, but you must also set `storeAccountKey=false` (the controller rejects the default `storeAccountKey=true` when shared key access is disabled) and use identity-based SMB mount (see [managed identity mount](managed-identity-mount.md)). Without an identity-based mount configured, SMB provisioning/mount will fail. | `true`,`false` | No | `true`
