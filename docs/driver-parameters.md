...
--- | **Following parameter applies to both SMB and NFS protocols** | --- | --- |
allowSharedKeyAccess | Allow or disallow shared key access for storage account created by driver. Applies to both SMB and NFS accounts. <br><br>**NFS:** safe to disable — NFS mount does not use the account key. <br><br>**SMB:** disabling shared key access is supported when combined with identity-based SMB mount — set one of `mountWithManagedIdentity=true`, `mountWithWorkloadIdentityToken=true`, or `mountWithOAuthToken=true` (see [managed identity mount](managed-identity-mount.md)); the driver then automatically sets `storeAccountKey=false` and enables SMB OAuth. If none of those identity-based mount flags are set, the default `storeAccountKey=true` conflicts with `allowSharedKeyAccess=false` and the controller will reject the request. | `true`,`false` | No | `true`
