...
useDataPlaneAPI | specify whether use [data plane API](https://learn.microsoft.com/en-us/rest/api/storageservices/file-service-rest-api) for file share create/delete/resize, this could solve the SRP API throttling issue since data plane API has almost no limit.<br>`true`: use data plane API with shared key; **fails when the storage account has firewall / vnet rules or `publicNetworkAccess=Disabled`**.<br>`false` (default): use the ARM (SRP) management plane; not subject to storage account firewall.<br>`oauth` (supported from v1.33.0): use data plane API authenticated with an OAuth token. On AKS-managed CSI driver this works with private storage accounts (Selected networks / Public disabled / Private Endpoint) when the driver identity has the `Storage File Data Privileged Contributor` role and `networkAcls.bypass` includes `AzureServices` (default). See [private storage account support](#using-usedataplaneapi-oauth-against-private-storage-accounts) below. | `true`,`false`,`oauth` | No | `false`
...
Setting `useDataPlaneAPI: "oauth"` on a `StorageClass` or `VolumeSnapshotClass` allows CSI driver operations that must go through the storage account's data plane endpoint (`<account>.file.core.windows.net`) to succeed even when:

- `Public network access` is `Disabled`, or
- `Public network access` is `Enabled from selected networks` with no allowed IP / vnet, or
- The storage account is only reachable through a Private Endpoint.

This is because the OAuth token issued for the driver identity carries a trusted-service claim, which the Storage firewall recognizes as a bypass when the storage account's `networkAcls.bypass` includes `AzureServices` (the default).
...
4. **Storage account settings**:
   ...
   - Recommended: `allowSharedKeyAccess = false`, so the driver cannot silently fall back to SAS.
