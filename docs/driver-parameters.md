...
comment | comment stored on the [file share snapshot](https://learn.microsoft.com/en-us/azure/storage/files/storage-snapshots-files) as the `comment` metadata entry, this is the same field shown in the Azure portal snapshot list | any string, e.g. `"snapshot before database migration"` | No | ``
