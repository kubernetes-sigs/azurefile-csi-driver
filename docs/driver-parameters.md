...
useDataPlaneAPI | specify whether use [data plane API](https://github.com/Azure/azure-sdk-for-go/blob/master/storage/share.go) for file share create/delete/resize, this could solve the SRP API throltting issue since data plane API has almost no limit, while it would be broken when there is firewall setting on storage account | `true`,`false` | No | `false`
...
fsGroupChangePolicy | indicates how volume's ownership and permissions will be changed | `Always`(default), `OnRootMismatch`| No | `Always`
