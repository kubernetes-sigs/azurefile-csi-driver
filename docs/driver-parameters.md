...
useDataPlaneAPI | specify whether use [data plane API](https://learn.microsoft.com/en-us/rest/api/storageservices/file-service-rest-api) for file share create/delete/resize, this could solve the SRP API throttling issue since data plane API has almost no limit, while it would fail when there is firewall or vnet setting on storage account. `oauth` value is supported from v1.33.0, which uses OAuth token for data plane API authentication | `true`,`false`,`oauth` | No | `false`
